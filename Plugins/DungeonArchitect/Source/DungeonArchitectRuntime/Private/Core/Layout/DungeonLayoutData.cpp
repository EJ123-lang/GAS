...
FBox FDungeonLayoutUtils::CalculateBounds(const FDungeonLayoutDataChunkInfo& LayoutShapes, const FCalcBoundsSettings& Settings) {
	...
	for (const FDABoundsShapeCircle& CircleItem : LayoutShapes.Circles) {
		FVector Center = CircleItem.Transform.GetLocation();
		Bounds += Center;

		const float Radius = CircleItem.Radius * CircleItem.Transform.GetScale3D().X;
			
		Bounds += Center + FVector(Radius, 0, 0);
		Bounds += Center - FVector(Radius, 0, 0);
		Bounds += Center + FVector(0, Radius, 0);
		Bounds += Center - FVector(0, Radius, 0);
	}
