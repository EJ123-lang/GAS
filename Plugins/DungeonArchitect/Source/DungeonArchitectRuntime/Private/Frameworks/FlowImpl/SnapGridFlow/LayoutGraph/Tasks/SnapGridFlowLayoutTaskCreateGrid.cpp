...
void USnapGridFlowLayoutTaskCreateGrid::Execute(const FFlowExecutionInput& Input, const FFlowTaskExecutionSettings& InExecSettings, FFlowExecutionOutput& Output) const {
    ...
    FNegationVolumeContext NegationVolumeContext{};
    {
        ...
        USnapGridFlowModuleDatabase* ModuleDatabase = SGFLayoutGraphDomain.IsValid() ? SGFLayoutGraphDomain->GetModuleDatabase() : nullptr;
        USnapGridFlowModuleBoundsAsset* ModuleBoundsAsset = ModuleDatabase ? ModuleDatabase->ModuleBoundsAsset : nullptr;
    ...
    CreateGraph(Graph, GridSize, NegationVolumeContext);
...
void USnapGridFlowLayoutTaskCreateGrid::CreateGraph(UFlowAbstractGraphBase* InGraph, const FIntVector& InGridSize, const FNegationVolumeContext& InNegationVolumeContext) {
    ...
    for (int z = 0; z < InGridSize.Z; z++) {
        for (int y = 0; y < InGridSize.Y; y++) {
            for (int x = 0; x < InGridSize.X; x++) {
                ...
                if (FGuid* PrevNodeX = Nodes.Find(FIntVector(x - 1, y, z))) {
                    InGraph->CreateLink(*PrevNodeX, Node->NodeId);
                }

                if (FGuid* PrevNodeY = Nodes.Find(FIntVector(x, y - 1, z))) {
                    InGraph->CreateLink(*PrevNodeY, Node->NodeId);
                }

                if (FGuid* PrevNodeZ = Nodes.Find(FIntVector(x, y, z - 1))) {
