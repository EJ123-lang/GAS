...
public:
    ...
    static void BeginDrawCanvasToRenderTarget(UTextureRenderTarget2D* TextureRenderTarget, UCanvas*& Canvas,
        ...
        if (FTextureRenderTargetResource* RenderTargetResource = TextureRenderTarget->GameThread_GetRenderTargetResource()) {
            ...
            // Old code used FDrawEvent allocation and BEGIN_DRAW_EVENTF_GAMETHREAD/STOP_DRAW_EVENT_GAMETHREAD macros.
            // Newer engine versions do not expose FDrawEvent on FDrawToRenderTargetContext. Use scope-based tracing if available,
            // otherwise rely on ENQUEUE_RENDER_COMMAND for GPU tasks. We simply avoid using the draw event here.
...
#if WANTS_DRAW_MESH_EVENTS
                // Draw event handling removed: no-op for newer engine versions
#endif // WANTS_DRAW_MESH_EVENTS
