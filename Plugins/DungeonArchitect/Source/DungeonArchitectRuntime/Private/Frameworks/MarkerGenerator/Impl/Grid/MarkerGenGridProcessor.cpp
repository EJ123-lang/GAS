		...
		if (FMGGridSceneMarkerList* MarkerList = InExecutor->GetMarkerList()) {
			...
			for (const FMGGridPatternAssemblyRule& Rule : InPattern.Rules) {
				...
				if (MarkerHeights.Num() > 0) {
					int FixedHeight = 0;
					// Use FindByPredicate to pick the first value as the fixed height
					for (const auto& Pair : MarkerHeights) {
						FixedHeight = Pair.Value;
						break;
					}
					for (const auto& Entry : MarkerHeights) {
