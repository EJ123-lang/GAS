...
void AAuraGameModeBase::TravelToMap(UMVVM_LoadSlot* Slot)
{
	if (!Slot)
	{
		UE_LOG(LogAura, Error, TEXT("Slot is null"));
		return;
	}

	if (Slot->MapAssetName.IsEmpty())
	{
		UE_LOG(LogAura, Error, TEXT("MapAssetName is empty!"));
		return;
	}

	UE_LOG(LogAura, Warning, TEXT("Opening map: %s"), *Slot->MapAssetName);

	UGameplayStatics::OpenLevel(this, FName(Slot->MapAssetName));
}
